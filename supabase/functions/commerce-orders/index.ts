...
type OrderSummary = { order_id?: number; order_number?: string; total?: number; status?: string; payment_status?: string }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (request.method === 'GET') {
      ...
      if (id) {
        const data = await callRpc<unknown>(client, 'runtime_order_get', {
          p_order_id: integerParam(id, 'INVALID_ORDER_ID'),
        })
        return data === null ? jsonResponse({ error: { code: 'RESOURCE_NOT_FOUND', request_id } }, 404, request_id)
          : jsonResponse({ data }, 200, request_id)
      }
      const data = await callRpc<unknown>(client, 'runtime_order_list', {
    ...
    if (request.method === 'POST') {
      const data = await callRpc<OrderSummary>(client, 'runtime_order_create', {
    ...
    if (request.method === 'PATCH') {
      ...
      const data = await callRpc<OrderSummary>(client, 'runtime_order_update', {
        p_order_id: integerParam(String(input.id), 'INVALID_ORDER_ID'),
        p_patch: input.patch,
        p_idempotency_key: idempotencyKey(request),
        p_request_id: request_id,
      })
    ...
    if (request.method === 'DELETE') {
      const data = await callRpc<OrderSummary & { deleted?: boolean }>(client, 'runtime_order_delete', {
        p_order_id: integerParam(url.searchParams.get('id'), 'INVALID_ORDER_ID'),
        p_idempotency_key: idempotencyKey(request),
        p_request_id: request_id,
      })
      return data === null ? jsonResponse({ error: { code: 'RESOURCE_NOT_FOUND', request_id } }, 404, request_id)
