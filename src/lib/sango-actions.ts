...
const sangoProductSchema = z.object({
    ...
    thickness_mm: z.coerce.number().int().nullable().optional(),
    width_mm: z.coerce.number().int().nullable().optional(),
    length_mm: z.coerce.number().int().nullable().optional(),
    ac_rating: z.string().optional().default(''),
    warranty_years: z.coerce.number().int().nullable().optional(),
    ...
    price_unit: z.string().optional().default('m²'),
...
export async function createSangoProduct(data: any) {
    const validated = sangoProductSchema.safeParse(data)
    ...
    try {
        await prisma.sango_products.create({
            data: {
                sku: d.sku,
                name: d.name,
                slug: d.slug,
                ...
                ac_rating: d.ac_rating || null,
                warranty_years: d.warranty_years || null,
                ...
                sango_product_images: d.additional_image_urls.length > 0 ? {
...
export async function updateSangoProduct(id: number, data: any) {
    const validated = sangoProductSchema.safeParse(data)
    if (!validated.success) {
        return { errors: validated.error.flatten().fieldErrors }
    }
    ...
    try {
        await prisma.$transaction([
            prisma.sango_product_images.deleteMany({ where: { product_id: id } }),
            prisma.sango_products.update({
                where: { id },
                data: {
                    sku: d.sku,
                    name: d.name,
        ...
        if (d.additional_image_urls.length > 0) {
            await prisma.sango_product_images.createMany({
...
export async function deleteSangoProduct(id: number) {
    try {
        await prisma.sango_products.delete({ where: { id } })
        revalidatePath('/admin/sango')
        return { success: true }
    } catch {
