...
| Scheduler/free tier | Accepted architecture reference: Supabase `pg_cron` plus `pg_net`, with a one-minute schedule model. Accepted LEO-531 feasibility proof: `PUBLISH_TO_LIVE: PASS`; representative disposable end-to-end freshness was approximately **1.473 seconds**; the updated `workers.dev` version returned HTTP 200; therefore the **<=5 minute** contract passed as a feasibility proof. | Before enabling a schedule, verify current scheduler, invocation, database, egress, retry, and log limits for the intended plan. Calculate scheduled runs plus retries/catch-up against the verified allowance; later implementation or cutover issues must remeasure freshness for their exact candidate and environment. Unknown limits, a failed or missing exact-candidate measurement, runaway retries, or write-freeze state is `BLOCKED`. Keep the one-shot scheduler, advisory-lock/idempotency controls, failure evidence, and no silent catch-up. |
