...
export PGOPTIONS='-c role=dpg_readonly'
...
if ! psql "$DATABASE_URL" -X -q -A -t -v ON_ERROR_STOP=1 \
  -c "select status from dpg_control.free_tier_database_guard" \
