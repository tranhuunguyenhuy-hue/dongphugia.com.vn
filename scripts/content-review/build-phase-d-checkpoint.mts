...
async function main() {
    ...
    const records = phaseD.buildPhaseDRecords(source.products, checkpointCohort.map(row => ({ id: row.id, sku: row.sku, name: row.name, brand: row.brand, category: row.category, family: row.classification.family, descriptionHash: row.descriptionHash })), 'worker-019fd0c7-fab8-7170-96af-d9d9a46f1519')
    ...
    const manifest = { schemaVersion: 1, artifact: 'leo-493-phase-d-checkpoint', policyHash: POLICY_HASH, snapshotHash: cohort.snapshotHash, cohortHash: cohort.cohortHash, checkpointHash: cohort.checkpointHash, sourceHash: source.sourceHash, proposalHash: packageValue.proposalHash, packageHash: packageValue.packageHash, sourceCommit, bindingStatus: 'VALID', counts: packageValue.counts, quality: packageValue.quality, publicModel }
    const familyCounts = packageValue.counts.byFamily as Record<string, number>
    const report = `# LEO-493 Phase D — 30-product checkpoint\n\n- **Binding:** policyHash \`${POLICY_HASH}\`; snapshotHash \`${cohort.snapshotHash}\`; cohortHash \`${cohort.cohortHash}\`; checkpointHash \`${cohort.checkpointHash}\`; sourceHash \`${source.sourceHash}\`; proposalHash \`${packageValue.proposalHash}\`; packageHash \`${packageValue.packageHash}\`; source commit \`${sourceCommit}\`.\n- **Scope:** 30 products from the accepted final 240 cohort only; no remaining 210 products generated. Family counts: ${JSON.stringify(familyCounts)}.\n- **Editorial:** deterministic product-specific narratives use ${new Set(records.map(record => record.narrativeFamily)).size} narrative families; Before/After ratio min=${(packageValue.quality.beforeAfterRatio.min ?? 0).toFixed(3)}, max=${(packageValue.quality.beforeAfterRatio.max ?? 0).toFixed(3)}, average=${(packageValue.quality.beforeAfterRatio.average ?? 0).toFixed(3)}; repeated opening keys=${packageValue.quality.repeatedOpeningCount}, closing keys=${packageValue.quality.repeatedClosingCount}, section signatures=${packageValue.quality.repeatedSectionSignatureCount}; retained source-evidence rate=${(packageValue.quality.retainedEvidenceRate * 100).toFixed(1)}%.\n- **Holdout:** 24 products, stratified ${JSON.stringify(packageValue.manualHoldout.reduce<Record<string, number>>((result, row) => { result[row.family] = (result[row.family] || 0) + 1; return result }, {}))}; manual label is limited to directly inspected Bunny main images and is not manufacturer-current verification.\n- **Media:** ${String(packageValue.counts.media)} references classified; action counts ${JSON.stringify(packageValue.counts.byMediaAction)}; placement counts ${JSON.stringify(packageValue.counts.byMediaPlacement)}; duplicate fingerprints inherit the first decision; REMOVE is absent from After (${packageValue.counts.removeInAfter}). Hita remains manual-only and no Hita request/crawl/copy/rehost ran.\n- **Official status:** LEO-492 evidence remains unresolved; no row is claimed manufacturer-current.\n- **Blockers:** ${JSON.stringify(packageValue.quality.blockedReasons)}.\n- **Public artifact:** sanitized manifest only; no live media URLs or raw HTML.\n- **Private review:** the ignored package/dashboard carries direct Bunny preview for local review; it is not a public URL.\n\nThis is a checkpoint for Coordinator inspection. Do not scale the remaining 210 products until accepted.\n`
