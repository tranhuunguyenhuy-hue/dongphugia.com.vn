...
const bepProductSchema = z.object({
...
export async function createBepProduct(data: any) {
    const validated = bepProductSchema.safeParse(data)
    ...
    try {
        await prisma.bep_products.create({
            data: {
                sku: d.sku,
                name: d.name,
                slug: d.slug,
                ...
                bep_product_images: d.additional_image_urls.length > 0 ? {
...
export async function updateBepProduct(id: number, data: any) {
    const validated = bepProductSchema.safeParse(data)
    if (!validated.success) {
        return { errors: validated.error.flatten().fieldErrors }
    }
    ...
    try {
        await prisma.$transaction([
            prisma.bep_product_images.deleteMany({ where: { product_id: id } }),
            prisma.bep_products.update({
                where: { id },
                data: {
                    sku: d.sku,
                    name: d.name,
        ...
        if (d.additional_image_urls.length > 0) {
            await prisma.bep_product_images.createMany({
...
export async function deleteBepProduct(id: number) {
    try {
        await prisma.bep_products.delete({ where: { id } })
        revalidatePath('/admin/bep')
        return { success: true }
    } catch {
