/**
 * Phase 4D.2 + 1E.3: Apply Rename & Generate Redirect Map
 *
...
const CSV_PATH = path.join(__dirname, 'output/phase4-mixed-rename-preview.csv');
...
async function main() {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  PHASE 4D: APPLY RENAME — ${DRY_RUN ? '🔍 DRY RUN' : '🚀 EXECUTE'}`);
  ...
  const jsonPath = path.join(OUTPUT_DIR, 'phase4-mixed-redirects.json');
  ...
  const csvPath = path.join(OUTPUT_DIR, 'phase4-mixed-redirects.csv');
  ...
  // ──────────────────────────────────────────
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  PHASE 4D SUMMARY — ${DRY_RUN ? 'DRY RUN' : 'EXECUTED'}`);
  console.log(`${'='.repeat(60)}`);
  ...
  if (DRY_RUN) {
    ...
    console.log(`\n✅ Phase 4D EXECUTED successfully!`);
    console.log(`\n📋 NEXT STEPS:`);
