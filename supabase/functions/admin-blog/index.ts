...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if(request.method==='GET'){
      ...
      const data=id?await callRpc(client,'leo542_admin_blog_get',{p_post_id:integerParam(id,'INVALID_POST_ID')}):await callRpc(client,'leo542_admin_blog_list',{p_limit:paginationParam(url.searchParams.get('limit'),25,'INVALID_LIMIT',100),p_offset:paginationParam(url.searchParams.get('offset'),0,'INVALID_OFFSET',100000),p_status:url.searchParams.get('status')})
    ...
    if(request.method==='POST'||request.method==='PATCH'){
      ...
      const data=await callRpc(client,'leo542_admin_blog_put',{p_post_id:optionalIntegerParam(body.id===undefined?null:String(body.id),'INVALID_POST_ID'),p_expected_version:expectedVersion(request,body.expected_version),p_input:body.input,p_idempotency_key:idempotencyKey(request),p_request_id:request_id})
