...
import { createTBVSProduct, updateTBVSProduct } from '@/lib/tbvs-actions'
...
interface ProductType { id: number; name: string; slug: string; tbvs_subtypes: LookupItem[] }
interface Technology { id: number; name: string; slug: string; tbvs_brands?: { name: string } | null }

interface TBVSProductFormProps {
    product?: any
...
export function TBVSProductForm({
    product,
    productTypes,
    brands,
    materials,
    ...
    const [form, setForm] = useState({
        ...
        technology_ids: (
            product?.tbvs_product_technologies?.map((t: any) => t.technology_id) || []
        ) as number[],
    })
    ...
    const [uploadedImages, setUploadedImages] = useState<string[]>(() => {
        ...
        if (product?.tbvs_product_images) {
            product.tbvs_product_images.forEach((img: any) => {
    ...
    const filteredSubtypes = form.product_type_id
        ? productTypes.find((pt) => pt.id.toString() === form.product_type_id)?.tbvs_subtypes || []
        : []
    ...
    const toggleTechnology = (id: number) => {
        setForm((prev) => {
            const arr = prev.technology_ids
            return {
                ...prev,
                technology_ids: arr.includes(id) ? arr.filter((v) => v !== id) : [...arr, id],
            }
        })
    }
    ...
    return (
        <form onSubmit={onSubmit} className="space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-[#E4EEF2] p-6 space-y-5">
                ...
                <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                        URL: /thiet-bi-ve-sinh/[loai]/<span className="font-mono text-primary">{form.slug || 'slug'}</span>
                    </p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-[#E4EEF2] p-6 space-y-5">
                ...
                <div>
                    <label className={labelCls}>Công nghệ đặc trưng</label>
                        ...
                        {technologies.map((tech) => {
                            ...
                            return (
                                ...
                                >
                                    {tech.name}
                                    {tech.tbvs_brands && (
                                        <span className="ml-1 text-xs opacity-60">({tech.tbvs_brands.name})</span>
                                    )}
                                </button>
                            )
                        })}
                        ...
                        {specRows.map((row, idx) => (
                            <div key={idx} className="flex gap-3 items-center">
                                ...
                                <input
                                    ...
                                    placeholder="Giá trị (VD: 650 x 355 x 795 mm)"
                                />
                                <button
                                    type="button"
