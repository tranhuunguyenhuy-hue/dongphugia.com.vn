...
function renderStats(filteredCount) { const all = MODEL.products; const reviewed = all.filter((product) => productDecision(product) !== "PENDING").length; const media = all.flatMap((product) => product.media); const human = media.filter((item) => item.classification.action === "HUMAN_REVIEW").length; const keep = media.filter((item) => item.classification.action === "KEEP_VERIFIED").length; $("stats").innerHTML = '<div class="stat"><span>Products</span><strong>' + all.length + '</strong><small>exact approved manifest</small></div><div class="stat"><span>Product review progress</span><strong>' + reviewed + '/' + all.length + '</strong><small>local decisions</small></div><div class="stat"><span>HUMAN_REVIEW media</span><strong>' + human + '</strong><small>proposed v2.1 action</small></div><div class="stat"><span>KEEP media</span><strong>' + keep + '</strong><small>verified v2.1 action</small></div><div class="stat"><span>Visible products</span><strong>' + filteredCount + '</strong><small>active filters</small></div>'; }
...
function getFilters() { return { search: $("search").value.toLocaleLowerCase().trim(), brand: $("brand").value, category: $("category").value, status: $("status").value, risk: $("risk").value, origin: $("origin").value, action: $("action").value }; }
function filteredProducts() { const filter = getFilters(); return MODEL.products.filter((product) => { const haystack = [product.sku, product.name, product.brand, product.category].join(" ").toLocaleLowerCase(); const mediaMatches = product.media.some((media) => (!filter.origin || media.classification.origin === filter.origin) && (!filter.action || media.classification.action === filter.action)); return (!filter.search || haystack.includes(filter.search)) && (!filter.brand || product.brand === filter.brand) && (!filter.category || product.category === filter.category) && (!filter.status || productDecision(product) === filter.status) && (!filter.risk || product.mediaRisk === filter.risk) && mediaMatches; }); }
...
addOptions("brand", values("brand"), "All brands"); addOptions("category", values("category"), "All categories"); addOptions("risk", values("mediaRisk"), "All media risk"); addOptions("origin", [...new Set(MODEL.products.flatMap((product) => product.media.map((media) => media.classification.origin)))].sort(), "All origins"); addOptions("action", [...new Set(MODEL.products.flatMap((product) => product.media.map((media) => media.classification.action)))].sort(), "All proposed actions"); ["search","brand","category","status","risk","origin","action"].forEach((id) => $(id).addEventListener("input", render)); $("export").addEventListener("click", downloadExport); $("reset").addEventListener("click", resetDecisions); $("content-tab").addEventListener("click", () => showView("content")); $("media-tab").addEventListener("click", () => showView("media")); document.addEventListener("click", (event) => { const button = event.target.closest("[data-product-id]"); if (button && button.classList.contains("product-card")) { selectedId = Number(button.dataset.productId); showView("content"); render(); } }); render();
`
...
<body><main class="app"><header><div><div class="eyebrow">Offline · ${visibility === 'private' ? 'private local visual review' : 'PR-safe sanitized review'}</div><h1>LEO-489 Pilot Review Dashboard</h1><p class="subtle">Exact 20-product actual-data proposal package · browser-local decisions only · package <code>${escapeHtml(model.packageHash)}</code></p></div><div class="actions"><button class="primary" id="export">Export deterministic JSON</button><button class="danger" id="reset">Reset local decisions</button></div></header><div class="notice"><strong>Safety boundary:</strong> This file is offline and proposal-only. It never writes a server, database, product, CDN or analytics endpoint. Hita-hosted media stay unloaded until an explicit single-asset click in the ignored private dashboard.</div><section class="stats" id="stats"></section><section class="panel toolbar"><label>Search SKU, name, brand or category<input id="search" type="search" placeholder="e.g. INAX or SFV-802S"></label><label>Brand<select id="brand"></select></label><label>Category<select id="category"></select></label><label>Review status<select id="status"><option value="">All statuses</option><option>PENDING</option><option>KEEP</option><option>HUMAN_REVIEW</option><option>REJECT</option></select></label><label>Media risk<select id="risk"></select></label><label>Origin<select id="origin"></select></label><label>Proposed action<select id="action"></select></label></section><nav class="view-tabs" aria-label="Review mode"><button class="view-tab active" id="content-tab">Content review</button><button class="view-tab" id="media-tab">Media Review</button></nav><section class="layout" id="content-review"><aside class="panel list" id="product-list"></aside><article class="panel detail" id="detail"></article></section><section class="media-review hidden" id="media-review"></section></main><script>${DASHBOARD_SCRIPT(serializedModel(dashboardModel))}</script></body></html>\n`
}
