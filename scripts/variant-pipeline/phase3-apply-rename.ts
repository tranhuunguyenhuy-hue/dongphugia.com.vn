/**
 * Phase 3D.2 + 1E.3: Apply Rename & Generate Redirect Map
 *
...
const CSV_PATH = path.join(__dirname, 'output/phase3-lavabo-rename-preview.csv');
...
async function main() {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  PHASE 3D: APPLY RENAME — ${DRY_RUN ? '🔍 DRY RUN' : '🚀 EXECUTE'}`);
  ...
  const jsonPath = path.join(OUTPUT_DIR, 'phase3-lavabo-redirects.json');
  ...
  const csvPath = path.join(OUTPUT_DIR, 'phase3-lavabo-redirects.csv');
  ...
  // ──────────────────────────────────────────
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  PHASE 3D SUMMARY — ${DRY_RUN ? 'DRY RUN' : 'EXECUTED'}`);
  console.log(`${'='.repeat(60)}`);
