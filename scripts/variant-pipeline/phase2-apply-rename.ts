/**
 * Phase 2E.2 + 1E.3: Apply Rename & Generate Redirect Map
 *
...
const CSV_PATH = path.join(__dirname, 'output/phase2-sen-tam-rename-preview.csv');
...
async function main() {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  PHASE 2E: APPLY RENAME — ${DRY_RUN ? '🔍 DRY RUN' : '🚀 EXECUTE'}`);
  ...
  const jsonPath = path.join(OUTPUT_DIR, 'phase2-sen-tam-redirects.json');
  ...
  const csvPath = path.join(OUTPUT_DIR, 'phase2-sen-tam-redirects.csv');
  ...
  // ──────────────────────────────────────────
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  PHASE 2E SUMMARY — ${DRY_RUN ? 'DRY RUN' : 'EXECUTED'}`);
  console.log(`${'='.repeat(60)}`);
