...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const product = await getBepProductBySlug(productSlug)
    if (!product) return {}

    const title = product.seo_title || product.name
    const description = product.seo_description
        || (product.description ? product.description.slice(0, 160) : null)
        || `${product.name} — thiết bị bếp cao cấp chính hãng tại Đông Phú Gia Đà Lạt.`
    ...
    return {
        ...
        openGraph: {
            ...
            description,
            url: `/thiet-bi-bep/${product.bep_product_types?.slug}/${product.slug}`,
            images,
            type: "website",
        },
        twitter: {
...
export default async function ProductDetailPage({ params }: PageProps) {
    ...
    const relatedProducts = await getRelatedBepProducts(
    ...
    const jsonLd = {
        ...
        url: `${siteUrl}/thiet-bi-bep/${typeSlug}/${product.slug}`,
        brand: { "@type": "Brand", name: product.bep_brands?.name || "Đông Phú Gia" },
        offers: {
            "@type": "Offer",
            url: `${siteUrl}/thiet-bi-bep/${typeSlug}/${product.slug}`,
            priceCurrency: "VND",
                ...
                <div className="flex flex-col lg:flex-row gap-10 xl:gap-16">
                    ...
                    <div className="lg:w-[500px] xl:w-[600px] shrink-0">
                        <ProductImageGallery
                            ...
                            additionalImages={product.bep_product_images as any[]}
                            ...
                            <div className="flex items-center gap-4 text-sm text-gray-500">
                                ...
                                {product.bep_brands && (
                                    <>
                                        ...
                                        <span>Thương hiệu: <strong className="text-gray-900">{product.bep_brands.name}</strong></span>
                                    </>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-3 p-5 bg-[#f8fafc] rounded-xl mb-6 text-[15px]">
                            {product.bep_subtypes && (
                                <div className="flex flex-col">
                                    ...
                                    <span className="font-medium text-gray-900">{product.bep_subtypes.name}</span>
                                </div>
                            )}
                            {product.origins && (
                                <div className="flex flex-col">
