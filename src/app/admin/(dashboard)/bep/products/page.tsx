...
import { BepProductDeleteButton } from './bep-product-delete-button'

export default async function BepProductsPage({
    searchParams,
}: {
    ...
    const where: any = {}
    if (type) {
        where.bep_product_types = { slug: type }
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold">Thiết bị bếp</h1>
                    <p className="text-sm text-muted-foreground mt-0.5">{products.length} sản phẩm</p>
                </div>
                <Button asChild>
                    <Link href="/admin/bep/products/new">
            ...
            <div className="flex gap-2 flex-wrap">
                ...
                {productTypes.map((pt) => (
                    <Link
                        key={pt.id}
                        href={`/admin/bep/products?type=${pt.slug}`}
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex items-center justify-end gap-1">
                                        <Button asChild variant="ghost" size="sm">
                                            <Link href={`/admin/bep/products/${p.id}`}>
                                                <Pencil className="h-3.5 w-3.5 mr-1" />
                                                Sửa
                                            </Link>
                                        </Button>
                                        <BepProductDeleteButton id={p.id} name={p.name} />
                                    </div>
                                </td>
                            </tr>
                        ))}
