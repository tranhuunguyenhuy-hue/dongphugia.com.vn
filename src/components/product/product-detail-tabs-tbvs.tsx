...
interface ProductDetailTabsTBVSProps {
    description?: string | null;
...
export function ProductDetailTabsTBVS({
    description,
    features,
    specifications,
    technologies = [],
}: ProductDetailTabsTBVSProps) {
                ...
                {activeTab === 'specs' && (
                    <div className="space-y-10">
                        ...
                        {technologies.length > 0 && (
                            <div>
                                <h3 className="text-lg font-semibold text-gray-900 mb-4">Công nghệ tích hợp</h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {technologies.map(tech => (
                                        <div key={tech.id} className="p-4 rounded-xl border border-[#dcfce7] bg-[#f0fdf4] flex flex-col gap-2">
                                            <div className="flex items-center gap-2">
                                                {/* Fallback styling if no icon available */}
                                                <div className="w-6 h-6 rounded bg-[#22c55e] flex items-center justify-center text-white font-bold text-xs shrink-0">
                                                    {tech.name.charAt(0)}
                                                </div>
                                                <h4 className="font-semibold text-[#15803d] text-[15px]">
                                                    {tech.name}
                                                </h4>
                                            </div>
                                            {tech.description && (
                                                <p className="text-[14px] text-gray-700 leading-relaxed ml-8">
                                                    {tech.description}
                                                </p>
                                            )}
                                        </div>
